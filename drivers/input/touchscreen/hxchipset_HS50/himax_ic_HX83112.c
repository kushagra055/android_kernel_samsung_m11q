...
static uint8_t hx_read_ic_ver(void)
{
	uint8_t tmp_data[DATA_LEN_4];
	...
	tmp_data[2] = 0x11;
	tmp_data[3] = 0x2A;
	kp_himax_parse_assign_cmd(hx83104_ic_b9_en, tmp_addr, sizeof(tmp_addr));
	g_core_fp.fp_register_write(tmp_addr, DATA_LEN_4, tmp_data, false);
	...
	tmp_data[2] = 0xAA;
	tmp_data[3] = 0x00;
	kp_himax_parse_assign_cmd(hx83104_ic_eb_en, tmp_addr, sizeof(tmp_addr));
	g_core_fp.fp_register_write(tmp_addr, DATA_LEN_4, tmp_data, false);

	/* Read IC data */
	kp_himax_parse_assign_cmd(hx83104_b8_ic_pa2, tmp_addr, sizeof(tmp_addr));
	g_core_fp.fp_register_read(tmp_addr, DATA_LEN_4, tmp_data, 0);
	I("%s: B8 PA2 = %X\n", __func__, tmp_data[0]);
	ic_id = tmp_data[0];
...
static bool hx83112_chip_detect(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		|| (tmp_data[1] == 0x2f))) {
			if (tmp_data[1] == 0x2a) {
				if (hx_read_ic_ver() == 0x81)
					strlcpy(private_ts->chip_name,
					HX_83104A_SERIES_PWON, 30);
				else
					strlcpy(private_ts->chip_name,
